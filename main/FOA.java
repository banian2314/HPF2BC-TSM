...
public class FOA extends Algorithm {

	public double Temprature;
	public int NP = 2; // fruit fly swarm的数量
	public int S = 1; // 每个 fruit fly swarm 中个体的数量
	public int d = 3; // 领域结构结构的大小，或者搜索的大小
	public Hueristics hueristics;
	public int dmain[];
	public double tvalue = 0.1;

	public FOA(Initialize params, String instance, int typeTime) {
...
//		hueristics.HPF3(pop[0], feval);
		hueristics.PFS_double(pop[0], feval);
		for (int i = 1; i < NP; i++) {
...
//			}						// smellSeach2(pop, offspring, feva0l);						updateBestSofar(bestsofar, pop, feval);			// updateBestsofar2(bestsofar, pop, feval);//			for (int j = 0; j < NP; j++) {//				System.out.println(pop[j].fit);//			}//			System.out.println(bestsofar.fit);			end = System.currentTimeMillis();//			System.out.println("time " + (end - start));		}		recordResualt(bestsofar);		System.out.println("time " + (end - start));
	...
	public static void main(String[] args) {
		...
		FOA foa = new FOA(params, instance, 1);
		foa.run();
	}
